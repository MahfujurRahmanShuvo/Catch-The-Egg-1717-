...
void display() {
    ...
    if (state == MENU) {
        ...
        // 🥚 EGGS
        for (auto &e : eggs) {
            if (e.type==0) glColor3f(1,1,1);
            else if (e.type==1) glColor3f(0,0,1);
            else if (e.type==2) glColor3f(1,0.8,0);
            else if (e.type==3) glColor3f(0.3,0.2,0);
    ...
    if (state == PLAYING) {
        ...
        for (int i=0;i<eggs.size();i++) {
            ...
            if (eggs[i].y <= 80 &&
                ...
                else if (eggs[i].type==1) score+=5;
                else if (eggs[i].type==2) score+=10;
                else if (eggs[i].type==3) score-=10;
                else {
                    int p = rand()%3;
                    if (p==0) basketW=150;
                    else if (p==1) slowTimer=200;
                    else timeLeft+=5;
                }
...
void keyboard(unsigned char key,int x,int y) {
    ...
    if (state==MENU && key=='s')
        ...
        if (key=='a') {
            ...
            if (!isBlocked(newX)) basketX = newX;
        }
